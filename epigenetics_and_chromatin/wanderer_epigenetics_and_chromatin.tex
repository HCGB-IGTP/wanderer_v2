...
{\color{red}
\subsection*{Methylation and expression exploring}
To study the influence of DNA methylation on the expression of the closest gene, Wanderer performs a correlation analysis of the expression value (summarized by gene) with any of the CpGs of the region. The user can navigate through upstream, gene body or downstream probes as far as they are in the gene's vicinity.
...
{\color{red}
Wanderer also explores the gene expression between normals and tumors using a by-gene value. This summarization takes into account all the exons of the gene. As a result, a boxplot depicting the differences in its overall expression between normal and tumors is produced; a Wilcoxon rank sum test checks whether normal and tumors are statistically equivalent.\\
}
