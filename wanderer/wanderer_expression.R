  ...
  if(dim(exons2)[1] > 0){
    ...
    if(plotting){
      ...
      if(plotmean){
        ...
        #plot for Normals
        par(mai = par()$mai + c(2,0,1,0))
        boxplot(t(ddN2[,2:dim(ddN2)[2]]), at = exons2$exon_start, names = NULL, xlim = c(xmin, xmax), notch=TRUE,
        ...
        #plot for Tumors
        boxplot(t(ddT2[,2:dim(ddT2)[2]]), at = exons2$exon_start, names = NULL, xlim = c(xmin, xmax),  notch=TRUE,
