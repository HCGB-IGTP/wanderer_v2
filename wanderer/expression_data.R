...
expression_data <- function(con, geneName, geneNamesType, tissue){
  ...
  probesaux <- dbSendQuery(con, statement = paste0("select * from annotations.exons_annot where ", geneNamesType, " = '", geneName, "' limit 1"))
  ...
  if(dim(probesaux)[1] > 0){
    ...
    probes <- dbSendQuery(con, statement = paste0("select * from annotations.exons_annot
            where chr = '", probesaux[5], "' and exon_start > (select genestart as sstart from annotations.exons_annot
